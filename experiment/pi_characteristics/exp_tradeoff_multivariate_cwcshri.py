...
# To collect the outputs
gamma_list = np.linspace(1.5, 10, 10) 
...
for j in range(N_trials):
    ...
    for i, gamma in enumerate(gamma_list):
        ...
        criterion = cwcshri_objective(delta_ = delta, gamma_ = gamma, soften_ = 50
                                      , smoothfunction = 'tanh', returnseparatedloss = True)

        train = trainer(num_epochs = 2000, batch_size = 500, patience = 200) 
        ...
        train_loss_collect.append(train_loss_list)
        val_loss_collect.append(val_loss_list)
        ...
        saved_result = {'outputs_train':outputs_train_all, 'outputs_val':outputs_val_all
                        ...
                        , 'gamma': gamma_list, 'train_loss':all_trainloss_list, 'val_loss':all_valloss_list}

        filename = f'cwcshri_tradeoff_dgpmultivariate.pkl'
        # # Save
    ...
    all_trainloss_list.append(train_loss_collect)
    all_valloss_list.append(val_loss_collect)
    
##########################################
print('---------- Finished ----------')
filenamedone = f'cwcshri_tradeoff_dgpmultivariate_done.pkl'
##########################################
# # Save
