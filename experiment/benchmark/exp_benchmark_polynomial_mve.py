...
dict_path = os.path.join(datafolderpath, 'dgp_polynomial_100trials.pkl')
hyperparam_path = os.path.join(hyperparameterfolderpath, 'hyperparam_mve_dgpall.json')
...
train = MVE_trainer(num_epochs = 2000, batch_size = 500, patience = 200) #Set the trainer
...
train = MVE_trainer(num_epochs = 2000, batch_size = hyperparams['batch_size'], patience = hyperparams['patience']) #Set the trainer
torch.manual_seed(21) #Must have to initialize the network parameters
model = MVENet(input_size = X_input.shape[1], hidden_size = 100, output_size = 2)
...
for j in range(N_trials):
    ...
    train = MVE_trainer(num_epochs = 2000, batch_size = hyperparams['batch_size'], patience = hyperparams['patience']) 
    ...
    outputs_val = train.predict(X_val_sorted, model, delta_ = delta, ymean = torch.mean(y_train), ystd = torch.std(y_train))
    outputs_train = train.predict(X_train_sorted, model, delta_ = delta, ymean = torch.mean(y_train), ystd = torch.std(y_train))
    ...
    filename = f'mve_performance_dgppolynomial.pkl'
    # # Save
...
##########################################
print('---------- Finished ----------')
filenamedone = f'mve_performance_dgppolynomial_done.pkl'
##########################################
# # Save
